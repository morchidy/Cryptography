/**
 * @file aes128_enc.c
 * @brief AES-128 encryption implementation
 * 
 * Byte-oriented implementation with on-the-fly key schedule.
 * Features constant-time xtime operation for side-channel resistance.
 */

#include "aes128_enc.h"
#include "field_ops.h"
#include "key_schedule.h"

/**
 * @brief AES forward S-box
 */
const uint8_t S[256] = {
    
};

/**
 * @brief AES inverse S-box
 */
const uint8_t Sinv[256] = {
    
};

/**
 * @brief Perform one AES round transformation
 * 
 * Applies SubBytes, ShiftRows, MixColumns (optional), and AddRoundKey.
 * 
 * @param block State block (modified in place)
 * @param round_key Round key to XOR
 * @param lastround If 16, skip MixColumns; if 0, perform full round
 */
void aes_round(uint8_t block[AES_BLOCK_SIZE], uint8_t round_key[AES_BLOCK_SIZE], int lastround)
{
    int i;
    uint8_t tmp;

    /* SubBytes + ShiftRows combined */
    
    /* Row 0: no shift */
    block[0]  = S[block[0]];
    block[4]  = S[block[4]];
    block[8]  = S[block[8]];
    block[12] = S[block[12]];
    
    /* Row 1: left shift by 1 */
    tmp = block[1];
    block[1]  = S[block[5]];
    block[5]  = S[block[9]];
    block[9]  = S[block[13]];
    block[13] = S[tmp];
    
    /* Row 2: left shift by 2 */
    tmp = block[2];
    block[2]  = S[block[10]];
    block[10] = S[tmp];
    tmp = block[6];
    block[6]  = S[block[14]];
    block[14] = S[tmp];
    
    /* Row 3: left shift by 3 (or right shift by 1) */
    tmp = block[15];
    block[15] = S[block[11]];
    block[11] = S[block[7]];
    block[7]  = S[block[3]];
    block[3]  = S[tmp];

    /* MixColumns (skip if lastround=16) */
    for (i = lastround; i < 16; i += 4) {
        uint8_t *column = block + i;
        uint8_t tmp2 = column[0];
        tmp = column[0] ^ column[1] ^ column[2] ^ column[3];

        column[0] ^= tmp ^ xtime(column[0] ^ column[1]);
        column[1] ^= tmp ^ xtime(column[1] ^ column[2]);
        column[2] ^= tmp ^ xtime(column[2] ^ column[3]);
        column[3] ^= tmp ^ xtime(column[3] ^ tmp2);
    }

    /* AddRoundKey */
    for (i = 0; i < 16; i++) {
        block[i] ^= round_key[i];
    }
}

/**
 * @brief Encrypt a block with AES-128
 * 
 * @param block Input block (modified in place)
 * @param key Master encryption key
 * @param nrounds Number of rounds (1-10)
 * @param lastfull 1 for full last round, 0 to skip MixColumns in last round
 */
void aes128_enc(uint8_t block[AES_BLOCK_SIZE], 
                const uint8_t key[AES_128_KEY_SIZE], 
                unsigned nrounds, 
                int lastfull)
{
    uint8_t ekey[32];
    unsigned int i;
    int pk, nk;

    /* Initial round: AddRoundKey with master key */
    for (i = 0; i < 16; i++) {
        block[i] ^= key[i];
        ekey[i] = key[i];
    }
    
    /* Generate first round key */
    next_aes128_round_key(ekey, ekey + 16, 0);

    pk = 0;
    nk = 16;
    
    /* Main rounds */
    for (i = 1; i < nrounds; i++) {
        aes_round(block, ekey + nk, 0);
        pk = (pk + 16) & 0x10;
        nk = (nk + 16) & 0x10;
        next_aes128_round_key(ekey + pk, ekey + nk, i);
    }
    
    /* Final round */
    if (lastfull) {
        aes_round(block, ekey + nk, 0);
    } else {
        aes_round(block, ekey + nk, 16);
    }
}
